...
Elm.Scoreboard = Elm.Scoreboard || {};
Elm.Scoreboard.make = function (_elm) {
   "use strict";
   _elm.Scoreboard = _elm.Scoreboard || {};
   if (_elm.Scoreboard.values) return _elm.Scoreboard.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var view = F2(function (address,model) {
      var header$ = A2($Html.header,_U.list([]),_U.list([$Html.text("High Scores")]));
      var stringifiedScores = A2($List.map,$Basics.toString,model.scores);
      var scores = A2($Html.div,
      _U.list([]),
      A2($List.map,function (score) {    return A2($Html.div,_U.list([]),_U.list([$Html.text(score)]));},stringifiedScores));
      return A2($Html.div,_U.list([]),_U.list([header$,scores]));
   });
