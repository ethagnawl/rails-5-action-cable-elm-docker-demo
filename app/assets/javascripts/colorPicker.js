...
Elm.Model.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var GameObject = F6(function (a,b,c,d,e,f) {    return {initials: a,initialsSaved: b,answer: c,options: d,guess: e,score: f};});
   var initialGameObject = A6(GameObject,"",false,"rgb(0, 0, 0)",_U.list(["rgb(0, 0, 0)"]),"",0);
   return _elm.Model.values = {_op: _op,initialGameObject: initialGameObject,GameObject: GameObject};
};
...
Elm.Action.make = function (_elm) {
   ...
   var Noop = {ctor: "Noop"};
   var update = F3(function (portRequestNewGameObject,action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "GuessMade": var _p2 = _p0._0;
           var score = _U.eq(_p2,model.answer) ? _U.eq(_p2,model.guess) ? model.score : model.score + 5 : _U.cmp(model.score - 5,0) < 0 ? 0 : model.score - 5;
           var model = _U.update(model,{guess: _p2,score: score});
           return {ctor: "_Tuple2"
                  ,_0: model
                  ,_1: A2($Effects.map,function (_p1) {    return Noop;},$Effects.task(A2($Signal.send,portRequestNewGameObject.address,model)))};
         case "GameObjectReceived": var _p3 = _p0._0;
           return noFx(_U.update(model,{answer: _p3.answer,guess: "",options: _p3.options,score: model.score}));
         case "InitialsAdded": return noFx(_U.update(model,{initials: _p0._0}));
         case "InitialsSaved": return noFx(_U.update(model,{initialsSaved: true}));
         default: return noFx(model);}
   });
   var InitialsSaved = function (a) {    return {ctor: "InitialsSaved",_0: a};};
   var InitialsAdded = function (a) {    return {ctor: "InitialsAdded",_0: a};};
   var GuessMade = function (a) {    return {ctor: "GuessMade",_0: a};};
   var GameObjectReceived = function (a) {    return {ctor: "GameObjectReceived",_0: a};};
   return _elm.Action.values = {_op: _op
                               ,update: update
                               ,GameObjectReceived: GameObjectReceived
                               ,GuessMade: GuessMade
                               ,InitialsAdded: InitialsAdded
...
Elm.View.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var answerView = function (color) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "background-color",_1: color}
   ...
   var optionView = F2(function (address,color) {
      var guessCallback = $Action.GuessMade(color);
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("option")
              ,A2($Html$Events.onClick,address,guessCallback)
              ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "border",_1: "1px solid black"}
                                              ,{ctor: "_Tuple2",_0: "display",_1: "inline-block"}
                                              ,{ctor: "_Tuple2",_0: "padding",_1: "12px"}
                                              ,{ctor: "_Tuple2",_0: "width",_1: "33%"}]))]),
      _U.list([$Html.text(color)]));
   });
   var view = F2(function (address,model) {
      var debugView = A2($Html.div,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"debug: ",model.answer))]));
      var scoreView = A2($Html.div,
      _U.list([]),
      _U.list([$Html.text(A2($Basics._op["++"],
      "player: ",
      A2($Basics._op["++"],model.initials,A2($Basics._op["++"],", score: ",$Basics.toString(model.score)))))]));
      var rightOrWrong = !_U.eq(model.guess,"") ? _U.eq(model.guess,model.answer) ? "Right!" : "Wrong!" : "";
      var rightOrWrongView = A2($Html.div,_U.list([]),_U.list([$Html.text(rightOrWrong)]));
      var prompt = !_U.eq(model.guess,"") ? "" : "Pick a color!";
      var promptView = A2($Html.div,_U.list([$Html$Attributes.$class("prompt")]),_U.list([$Html.text(prompt)]));
      var options$ = A2($Html.div,_U.list([$Html$Attributes.$class("options")]),A2($List.map,optionView(address),model.options));
      var answer$ = answerView(model.answer);
      var initialsCallback = function (initials) {    return A2($Signal.message,address,$Action.InitialsAdded(initials));};
      var initialsView = A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.autofocus(true)
                      ,$Html$Attributes.placeholder("Enter your initials")
                      ,$Html$Attributes.value(model.initials)
                      ,A3($Html$Events.on,"input",$Html$Events.targetValue,initialsCallback)]),
              _U.list([]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,$Action.InitialsSaved(true))]),_U.list([$Html.text("Play!")]))]));
      return _U.eq(model.initialsSaved,false) ? A2($Html.div,_U.list([]),_U.list([initialsView])) : A2($Html.div,
      _U.list([]),
      _U.list([answer$,promptView,options$,rightOrWrongView,scoreView,debugView]));
   });
   return _elm.View.values = {_op: _op,optionView: optionView,answerView: answerView,view: view};
};
Elm.ColorPicker = Elm.ColorPicker || {};
Elm.ColorPicker.make = function (_elm) {
   "use strict";
   _elm.ColorPicker = _elm.ColorPicker || {};
   if (_elm.ColorPicker.values) return _elm.ColorPicker.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var view = F2(function (address,model) {    return A2($View.view,address,model);});
   var options = Elm.Native.Port.make(_elm).inboundSignal("options",
   "Model.GameObject",
   function (v) {
      return typeof v === "object" && "initials" in v && "initialsSaved" in v && "answer" in v && "options" in v && "guess" in v && "score" in v ? {_: {}
                                                                                                                                                   ,initials: typeof v.initials === "string" || typeof v.initials === "object" && v.initials instanceof String ? v.initials : _U.badPort("a string",
                                                                                                                                                   v.initials)
                                                                                                                                                   ,initialsSaved: typeof v.initialsSaved === "boolean" ? v.initialsSaved : _U.badPort("a boolean (true or false)",
                                                                                                                                                   v.initialsSaved)
                                                                                                                                                   ,answer: typeof v.answer === "string" || typeof v.answer === "object" && v.answer instanceof String ? v.answer : _U.badPort("a string",
                                                                                                                                                   v.answer)
                                                                                                                                                   ,options: typeof v.options === "object" && v.options instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.options.map(function (v) {
                                                                                                                                                      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
                                                                                                                                                      v);
                                                                                                                                                   })) : _U.badPort("an array",
                                                                                                                                                   v.options)
                                                                                                                                                   ,guess: typeof v.guess === "string" || typeof v.guess === "object" && v.guess instanceof String ? v.guess : _U.badPort("a string",
                                                                                                                                                   v.guess)
                                                                                                                                                   ,score: typeof v.score === "number" && isFinite(v.score) && Math.floor(v.score) === v.score ? v.score : _U.badPort("an integer",
                                                                                                                                                   v.score)} : _U.badPort("an object with fields `initials`, `initialsSaved`, `answer`, `options`, `guess`, `score`",
      v);
   });
   var incomingGameObject = A2($Signal.map,$Action.GameObjectReceived,options);
   var portRequestNewGameObject = $Signal.mailbox($Model.initialGameObject);
   var requestNewGameObject = Elm.Native.Port.make(_elm).outboundSignal("requestNewGameObject",
   function (v) {
      return {initials: v.initials
             ,initialsSaved: v.initialsSaved
             ,answer: v.answer
             ,options: Elm.Native.List.make(_elm).toArray(v.options).map(function (v) {    return v;})
             ,guess: v.guess
             ,score: v.score};
   },
   portRequestNewGameObject.signal);
   var update = $Action.update(portRequestNewGameObject);
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: $Model.initialGameObject,_1: $Effects.none}
                             ,view: view
                             ,update: update
                             ,inputs: _U.list([incomingGameObject])});
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var main = app.html;
   return _elm.ColorPicker.values = {_op: _op
                                    ,portRequestNewGameObject: portRequestNewGameObject
                                    ,incomingGameObject: incomingGameObject
                                    ,view: view
                                    ,update: update
                                    ,app: app
                                    ,main: main};
